<div>
    ...
    {{-- <div class="divide-x divide-black text-sm leading-6 text-gray-500 grid grid-flow-col auto-cols-max h-full">
        <div class="col-end-1 w-14 z-10"></div>

        @foreach($projects as $project_index => $project)
            <div class="w-48 p-3 border-r-4 h-full">
            </div>
        @endforeach
    </div> --}}

    {{-- HORIZONTAL LINES HERE --}}
    <div class="flex flex-auto overflow-x-auto divide-x divide-gray-100" x-bind="scrollSync">
        <div class="sticky left-0 z-20 w-14 flex-none bg-white ring-1 ring-gray-100"></div>
        <div>
            <div style="width: 2300px;" class="divide-y divide-blue-200 -mt-1">
                @foreach($days as $day_index => $day)
                    ...
                    {{--  bg-indigo-100 bg-opacity-10 --}}
                    <div class="grid-stack mb-2 -mt-10"
                        x-data="{
                            init() {
