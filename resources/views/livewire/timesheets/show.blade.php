                ...
                <x-cards.body>
                    <x-lists.ul>
                        <x-lists.search_li
                            ...
                            href="{{route('users.show', $timesheet->user)}}"
            ...
            <x-cards.wrapper class="col-span-4 lg:col-span-2 lg:col-start-3">
                ...
                <x-lists.ul>
                    @foreach($weekly_hours as $timesheet)
                        ...
                        <x-lists.search_li
                            href="{{$timesheet->check ? route('checks.show', $timesheet->check->id) : (!$timesheet->check && $timesheet->check_id ? '' : (auth()->user()->primary_vendor->pivot->role_id == 1 ? route('timesheets.payment', $timesheet->user_id) : ''))}}"
                            :line_details="$line_details"
                            :line_title="$timesheet->project->name"
                            :bubble_message="$timesheet->paid_by ? 'Paid By' : ($timesheet->check_id ? 'Paid' : (auth()->user()->primary_vendor->pivot->role_id == 1 ? 'Pay' : 'Not Paid'))"
